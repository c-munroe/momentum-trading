...
def get_strategy_diversification_group(strategy_name, strategy_metadata):
    if strategy_name not in strategy_metadata.index:
        return (None, None, None)

    row = strategy_metadata.loc[strategy_name]
...
def select_diversified_strategies(ranked_candidates, strategy_metadata, target_count=5):
    selected = []
    seen_groups = set()

    for strategy_name in ranked_candidates.index:
        group = get_strategy_diversification_group(
            strategy_name=strategy_name,
            strategy_metadata=strategy_metadata,
        )
...
def calculate_walk_forward_ensemble_validation(
    returns_frame,
    strategy_metadata,
    train_months=120,
    test_months=12,
    ...
    returns_frame = returns_frame.dropna(how="all")
    methods = {
        "Top-3 ensemble": {"target_count": 3, "diversified": False},
        "Top-5 ensemble": {"target_count": 5, "diversified": False},
        "Diversified Top-5 ensemble": {"target_count": 5, "diversified": True},
    }
    ensemble_returns = {
        method: []
    ...
    for test_start in range(train_months, len(index), test_months):
        ...
        for method_name, method_settings in methods.items():
            if method_settings["diversified"]:
                selected_strategies = select_diversified_strategies(
                    ranked_candidates=ranked_candidates,
                    strategy_metadata=strategy_metadata,
                    target_count=method_settings["target_count"],
                )
            else:
                selected_strategies = ranked_candidates.head(
                    method_settings["target_count"]
                ).index.tolist()

            if not selected_strategies:
                continue

            selected_test_returns = test_slice[selected_strategies].dropna(how="any")

            if selected_test_returns.empty:
                continue

            combined_returns = selected_test_returns.mean(axis=1)
            test_metrics = calculate_performance_metrics(combined_returns)
            train_sharpes = ranked_candidates.loc[
                selected_strategies,
                "sharpe_ratio",
            ]
            ensemble_returns[method_name].append(combined_returns)
            decisions.append(
                {
                    "method": method_name,
                    "test_year": combined_returns.index.min().year,
                    "train_start": train_slice.index.min(),
                    "train_end": train_slice.index.max(),
                    "test_start": combined_returns.index.min(),
                    "test_end": combined_returns.index.max(),
                    "selected_strategy_names": " ; ".join(selected_strategies),
                    "selected_train_sharpes": ", ".join(
                        f"{value:.3f}"
                        for value in train_sharpes
                    ),
                    "ensemble_test_return": test_metrics["cumulative_return"],
                    "ensemble_test_sharpe_ratio": test_metrics["sharpe_ratio"],
                    "ensemble_test_max_drawdown": test_metrics["max_drawdown"],
                    "ensemble_test_calmar_ratio": test_metrics["calmar_ratio"],
                }
            )

    combined_ensemble_returns = {}

    for method_name, return_parts in ensemble_returns.items():
        if not return_parts:
            combined_ensemble_returns[method_name] = pd.Series(dtype=float)
            continue

        combined = pd.concat(return_parts).sort_index()
        combined = combined[~combined.index.duplicated(keep="first")]
        combined_ensemble_returns[method_name] = combined

    return combined_ensemble_returns, pd.DataFrame(decisions)
